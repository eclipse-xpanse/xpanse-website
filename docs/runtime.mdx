...
1. Configuration properties of authentication layer.
   Documented [here](authentication-authorization.mdx#runtime).
2. Configuration properties of database layer. Documented <Link name={'here'} url={'./database.mdx#maria-db'} isOpenInNewTab={true}/>.
3. Plugin activation variables. Documented <Link name={'here'} url={'./plugins#plugin-activation'}/>.

### Terraform CLI Installation

Terraform CLI can be installed using the steps from
the <Link name={'official guide'} url={'https://developer.hashicorp.com/terraform/downloads'}/>.
...
or <Link name={'temurin'} url={'https://adoptium.net/'}/>.
