...
xpanse uses <Link name={'OPA - Open Policy Agent'} url={'https://www.openpolicyagent.org/'}/> for this purpose.
...
found <Link name={'here'} url={'https://www.openpolicyagent.org/docs/latest/policy-language/'}/>.
...
-   As an integration test to validate if the service is deploying exactly what it's supposed to do.
-   and many more.
...
xpanse uses an application called <Link name={'policy-man'} url={'https://github.com/eclipse-xpanse/policy-man'}/> for the below purposes
