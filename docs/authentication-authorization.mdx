...
More details can be found <Link name={'here.'} url={'./runtime'}/>.
...
In order to allow service vendors to manage only their own services, we use ABAC. This is achieved by using `metadata` feature of OIDC to assign `isv` users to a specific namespace.
...
With this, users of another namespace won't be allowed to view, update or delete these service templates.
