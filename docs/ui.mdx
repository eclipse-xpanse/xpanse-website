...
For local environments, the `access_tokens` are stored with in the browser storage. This is not safe, but it is enough for non-production environments and for debugging purposes.
...
`oidcDomains` must have the identity provider URL and `accessTokenDomains` must have the Xpanse backend server URL.
