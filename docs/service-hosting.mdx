...
Service hosting type refers to the cloud provider account/tenant that must be used for deploying the managed service.
...
This is means, when the service vendor want to offer service on multiple hosting types, then one service template must be registered
